        ...
        {products.map((item) => (
          <li key={item.id} style={{ marginBottom: '20px', borderBottom: '1px solid #ccc' }} >
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button onClick={()=>cartService.add(item)} className='bg-(--accent) text-(--text) py-1 px-3'>Add</button>
            </div>
            <p style={{ fontSize: '0.9em', color: '#666' }}>{item.description}</p>
            <em style={{ fontSize: '0.8em' }} className='text-blue-400'>{item.isAvailable}</em>
          </li>
        ))}
      </ul>
    </div>
